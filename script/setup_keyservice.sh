...
# 参数控制：是否显示终端窗口
# 用法: bash setup_keyservice.sh [--show-terminals]
...
echo ""
echo "=== 设置脚本权限 ==="
chmod +x ./slave/key_service/arm_service/script/init.sh 2>/dev/null && echo "✓ arm_service init.sh 权限已设置" || echo "⚠️  arm_service init.sh 权限设置失败"
...
# 启动arm控制节点（独立终端）
run_service "ARM CONTROL" "$PWD" "source install/setup.bash; echo '=== 启动arm控制节点 ==='; ros2 run arm_service ctrl_node; exec bash"
...
run_service "ORBBEC CAMERA" "$PWD" "source install/setup.bash; ros2 launch orbbec_camera astra.launch.py; exec bash"
cd ../../../../
