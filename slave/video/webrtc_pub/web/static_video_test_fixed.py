                        ...
                        if candidate:
                            try:
                                ...
                                try:
                                    ...
                                    parsed_candidate = candidate_from_sdp(candidate.get("candidate"))
                                    parsed_candidate.sdpMid = candidate.get("sdpMid")
                                    parsed_candidate.sdpMLineIndex = candidate.get("sdpMLineIndex")
                                    await pc.addIceCandidate(parsed_candidate)
                                    print("✅ ICE candidate已添加 (第二次尝试)")
                                except Exception as e2:
                                    print(f"⚠️  第二次尝试也失败: {e2}")
                        else:
                            print("⚠️  收到空的ICE candidate")
